...
Проект находится на стадии активной разработки (MVP).

### ✅ Реализовано:
- **Управление профилями:** Полный CRUD для данных разработчика, навыков, опыта работы и проектов.
- **Гибкая инфраструктура:** Поддержка PostgreSQL (EF Core) и MongoDB с переключением через конфиг.
- **Качество:** Высокое покрытие интеграционными тестами бизнес-сценариев.
- **Документация:** Автоматическая генерация API Docs (Scalar/OpenAPI) по адресу `/docs`.

### ⚠️ Ограничения (в разработке):
- **Безопасность:** Аутентификация (JWT) требует финальной настройки в DI. Эндпоинты помечены `[Authorize]`, но механизмы проверки временно не активны.
- **Отказоустойчивость:** Отсутствует глобальная обработка ошибок и стандарт Problem Details.
- **Мониторинг:** Не внедрены Health Checks и структурированное логирование.
