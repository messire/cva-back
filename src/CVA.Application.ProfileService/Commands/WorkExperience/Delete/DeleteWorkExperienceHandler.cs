...
public sealed class DeleteWorkExperienceHandler(IDeveloperProfileRepository repository, ICurrentUserAccessor userAccessor) 
    : ICommandHandler<DeleteWorkExperienceCommand, ProfileDto>
{
    /// <inheritdoc />
    public async Task<Result<ProfileDto>> HandleAsync(DeleteWorkExperienceCommand command, CancellationToken ct)
    {
